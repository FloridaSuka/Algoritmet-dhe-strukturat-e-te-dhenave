...
//5)Two Sum
    vector<int> twoSum(vector<int>& nums, int target) {
        unordered_map<int, int> map; // Harta për ruajtjen e numrave dhe indekseve të tyre
        vector<int> result;
        for (int i = 0; i < nums.size(); i++) {
            int complement = target - nums[i];
            // Kontrollon nëse ekziston komplementi në hartë
            if (map.find(complement) != map.end()) {
                result.push_back(map[complement]);
                result.push_back(i);
                return result;
            }
            // Ruaj indeksin e çdo elementi në hartë
            map[nums[i]] = i;
        }
